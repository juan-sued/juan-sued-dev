...
export default async function CertificationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminShell email={admin.email}>
      ...
      <p className="mt-2 text-sm text-[var(--muted)]">{certification.issuer} · {certification.category} · {new Intl.DateTimeFormat("pt-BR", { dateStyle: "long" }).format(new Date(`${certification.completed_at}T00:00:00Z`))}</p>
